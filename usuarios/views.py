...
def login(request):
    if request.method == 'GET':
        ...
        if user:
            ...
            return render(request, 'usuarios/home.html')  # redirecionar para a home do site
        else:
...
def lancar(request):
    if request.method == 'GET':
        ...
        if nota_verificada:
            return HttpResponse('Disciplina já possuí nota cadastra!')
        else:
            nota.save()
            #return HttpResponse('Nota lançada com sucesso!')
            return render(request, 'usuarios/home.html')
...
def logout(request):
    if request.user.is_authenticated:
        ...
        #return HttpResponse('Sessão encerrada!')
        return render(request, 'usuarios/login.html')
    else:
        return HttpResponse('Faça o login para acessar a página!')